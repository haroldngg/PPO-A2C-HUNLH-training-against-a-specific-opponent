...
class RLCardBase(AECEnv):
    def __init__(self, name: str, num_players: int, obs_shape: tuple, obs_type):
        super().__init__()
        self.name = name
        ...
        max_round = 4  # par exemple, pour le Texas Hold'em: 0-Pre-flop, 1-Flop, 2-Turn, 3-River
        max_last_action = 8  # le nombre d'actions possibles
        self.obs_shape = obs_shape
        self.obs_type = obs_type
        self.obs_shape_numerical = (int(self.obs_shape[0])+2,)
                ...
                spaces.Dict(
                    {
                        "observation": spaces.Box(
                            low=np.array([0.0] *(self.obs_shape_numerical[0] - 2) + [0, 0]), high=np.array([1.0] *(self.obs_shape_numerical[0]-2) + [max_round, max_last_action]), dtype=self._dtype
                        ),
    ...
    def observe(self, agent):
        #obs = self.env._extract_state(self.env.get_state(self._name_to_int(agent)))
        obs = self.env.get_state(self._name_to_int(agent))
        #observation = obs["observation"].astype(self._dtype)
    ...
    def reset(self, seed=None, options=None):
        ...
        try:
            ...
            print("Key 'observation' not found in the observation data:", obs)
            self._last_obs = np.zeros(self.obs_shape_numerical)
